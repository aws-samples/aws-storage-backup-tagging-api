...
@given(u'the api /v1/accounts/<account>/regions/<region>/resourceType/storage/tags exists')
...
@given(u'the resourceType in url is invalid')
def step_impl(context):
    """
    Mark the resourceType in the url to compute. Compute is an unsupported resourceType for this API
    """
    account = config['main']['account']
    region = config['main']['region']
    vpce = config['behave']['vpc_endpoint']
    ...
    context.url = f'https://{vpce}/{env}/v1/accounts/{account}/regions/{region}/resourceType/compute'
    print(context.url)
    logger.info('--------------------------')
    logger.info(context.url)
    logger.info('--------------------------')
...
@given(u'the request body contains valid tags list')
def step_impl(context):
    """
    ...
    :return:
    """
    context.request_body = context.bdd_tag_list


@given(u'the request body contains invalid tags list')
def step_impl(context):
    """
        Set invalid tag keys in the request body
        :param context:
        :return:
        """
    context.request_body = context.bdd_tag_list
    context.request_body['vpcx-backup'] = "invalid_value"


@given(u'the account has at least one resource for each storage resource type')
def step_impl(context):
    """
    Make sure resources are created in the test account.
    """
    logger.info("Storage resources created in the before_all hook.")
...
@then(u'all the storage resources in the account tagged with the tag list in the request')
def step_impl(context):
    """
    ...
    for key in context.bdd_tag_list:
        expected_tags.append({
            'Key': key,
            'Value': context.bdd_tag_list.get(key)
        })
