...
def before_all(context):
    ...
    context.bdd_resource_arn_list.append(
        f"arn:aws:redshift:{context.region}:{context.test_account_id}:cluster:{context.cluster_identifier}"
    )
    context.ebs_volumes_ids = [context.volume_id]
    context.exception_request_body = {
        'resource_arns': context.bdd_resource_arn_list,
        'volume_ids': context.ebs_volumes_ids
    }
    context.enable_bdd_tag_list = {
        'vpcx-skip-backup': 'true'
    }
    context.disable_bdd_tag_list = {
        'vpcx-skip-backup': 'false'
    }
...
def after_scenario(context, scenario):
    ...
    if scenario.name == "Negative Scenario - Enable Unsuccessful when Legal Hold":
        resource_arn_to_un_tag_legal_hold = context.bdd_resource_arn_list[0]
        ...
        try:
            context.resource_tagging_client.untag_resources(
                ResourceARNList=[
                    resource_arn_to_un_tag_legal_hold
                ],
                TagKeys=[
                    'vpcx-backup'
                ]
